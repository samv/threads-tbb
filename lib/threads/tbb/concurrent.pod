...
different thread, then the deep copy happens.

The problem with that is that the deep copy is not a perl internals
thing; it requires running C<Storable::dump>, but passing in foreign
pointers.  This might work or it might be a recipe for segfaults; to
be confirmed via experiment.
