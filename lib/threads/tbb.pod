...
=head1 SYNOPSIS
 ...
 sub my_callback {
     ...
     for my $idx ($int_range->begin .. $int_range->end-1) {
         ...
         $self->{output}->STORE($idx, $transmuted);
     }
 }


 sub results {  # coming soon: no need for this hack!  :)
     my $self = shift;
     my @res;
     my $top = $self->{output}->FETCHSIZE-1;
     for (my $i = 0; $i <= $top; $i++) {
         push @res, $self->{output}->FETCH($i);
     }
 }
 ...
 unless ($threads::tbb::worker) {  # single script uses can use this
     ...
     say $_ for $parallel_transmogrificator->results();
 }
