...
=head1 SYNOPSIS
 ...
 unless ($threads::tbb::worker) {  # single script uses can use this
     ...
     # a concurrent array: lazy, deep copy in and out
     tie my @data, "threads::tbb::concurrent::array";

     push @data, @contents;    # coming soon: @data = @contents;

     my $range_i = threads::tbb::blocked_int->new(1, $#data + 1, 5);

     # make a "body function" which does the work.
     # coming soon: support for closures!
     my $body = $tbb->for_int_array_func( \@data, "Some::Func" );

     # run the body in parallel
     $tbb->parallel_for($range_i, $body);
 }

 # this is the body function's callback.
 sub Some::Func {
     my $body = shift;
     my $range = shift;
     my $array = shift;
     ...
     for my $idx ($range->begin .. $range->end-1) {
         ...
         # remember to put it back, or your changes might
         # not be saved.
         $array->[0] = $item;
     }
 }
...
modules hardly ever implement anyway.  Instead, each interpreter must
load all of the modules required to get it to do useful work on its
own.  This is largely automatic, however you should read the
...
structures between threads; see L<threads::tbb::concurrent> for more
information.
...
context.  Workers should have the same module set as "now".

  use threads::tbb;
  my $tbb = threads::tbb->new();

At compile time, the library takes a copy of the %INC global variable
(see L<perlvar/%INC>).  It also saves and places a special callback
onto the @INC global (see L<perlfunc/require>) which records all of
the modules later loaded by code.
...
Memory allocation by Perl core will both suffer from contention (as
all threads must use the memory allocator in turn) and from false
sharing on SMP systems due to insufficient allocation alignment.  That
is, blocks smaller than the smallest unit of cache the processor can
"own" are allocated.
...
B<range> object, and a B<body> object.
...
Then, you can declare a range over the keys of that container (eg, a
range of integers).  Finally, call C<parallel_for> and pass it the
range and a closure which can make reference to the concurrent
container.
